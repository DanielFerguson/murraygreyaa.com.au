...
$section_cta_label = get_field( 'section_cta_label' );
$section_cta_url   = get_field( 'section_cta_url' );
		...
		<div class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
			<div class="max-w-2xl">
				<?php if ( $section_heading ) : ?>
					<h2 class="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl"><?php echo esc_html( $section_heading ); ?></h2>
				<?php endif; ?>
				<?php if ( $section_intro ) : ?>
					<p class="mt-3 text-lg leading-8 text-slate-600"><?php echo wp_kses_post( nl2br( $section_intro ) ); ?></p>
				<?php endif; ?>
			</div>
			<?php if ( $section_cta_label && $section_cta_url ) : ?>
				<a class="inline-flex items-center justify-center rounded-full border border-slate-300 px-5 py-2 text-sm font-semibold text-slate-700 transition hover:border-brand hover:text-brand" href="<?php echo esc_url( $section_cta_url ); ?>">
		...
		<?php if ( ! empty( $posts ) ) : ?>
			<div class="mt-10 grid gap-8 md:grid-cols-3">
				<?php foreach ( $posts as $post ) : ?>
					<article class="group flex h-full flex-col rounded-3xl border border-slate-200/70 bg-white/90 p-6 shadow-sm shadow-slate-200/60 transition hover:-translate-y-1 hover:border-brand/40 hover:shadow-lg hover:shadow-brand/20">
